...
class TLSyncRoom {
  ...
  handlePushRequest(session, message) {
    ...
    const addDocument = (storage, changes2, id, _state, authorize, prevDoc) => {
      ...
      let { value: state } = res;
      const doc = prevDoc !== void 0 ? prevDoc ?? void 0 : storage.get(id);
      if (authorize) {
    ...
    const patchDocument = (storage, changes2, id, patch, authorize, prevDoc) => {
      const doc = prevDoc ?? storage.get(id);
      if (!doc) return;
      const recordType = (0, import_utils.assertExists)((0, import_utils.getOwnProperty)(this.schema.types, doc.typeName));
          ...
          for (const [id, op] of (0, import_utils.objectMapEntriesIterable)(message.diff)) {
            switch (op[0]) {
              case import_diff.RecordOpType.Put: {
                ...
                const record = op[1];
                const prevRecord = session ? txn.get(id) ?? null : void 0;
